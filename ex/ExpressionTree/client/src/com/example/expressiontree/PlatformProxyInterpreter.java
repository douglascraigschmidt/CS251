...
public class PlatformProxyInterpreter {
	...
	/**
	 * Interprets the given server response and makes the corresponding calls to Platform.
	 */
	public static void interpret(Platform platform, ServerResponse response) {
		if (response.result.toLowerCase().equals("ok")) {
			for (PlatformCall call : response.platformCalls) {
				...
				if (call.fname.equals("outputMenu"))
					platform.outputMenu(call.params.get(0), call.params.get(1), call.params.get(2));
				...
				if (call.fname.equals("errorLog"))
					// @@ TODO - Is this secure?
					platform.errorLog(call.params.get(0), call.params.get(1));
