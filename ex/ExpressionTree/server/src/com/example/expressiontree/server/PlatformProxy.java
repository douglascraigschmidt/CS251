...
public class PlatformProxy extends Platform {
	 ...
	 * list.
	 */
	ThreadLocal<List<PlatformCall>> mCallObjects = new ThreadLocal<List<PlatformCall>>() {
		@Override
	 ...
	 * 
	 * This method is called "reap" because the Platform accrues calls made to
	 * it over time and stores them in memory as PlatformCall objects. Then,
	 * when the server is ready to send a response back to the client, it
	 * collects, or "reaps", the accrued calls into a single ServerResponse
	 * object. Each time reap is called, the list of PlatformCalls is cleared
	 * out, so that any given call made to this Platform will only be returned
	 * by reap() once.
	 */
