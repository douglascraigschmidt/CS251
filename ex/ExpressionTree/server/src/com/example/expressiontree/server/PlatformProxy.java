 ...
 *        instances of ServerResponse, which can then be converted
 *        into GSON and sent back to the client.
 * 
 *        This class also uses the Thread-Specific Storage pattern to
 *        avoid collisions when processing requests from multiple
 *        threads at once.
 */
public class PlatformProxy extends Platform {
    /**
     * The list of platform calls made to this class. Each thread has
     * it's own list.

     * @@ Mitchell, can you please try to use the Java ThreadLocal
     * storage class for this since it does essentially the same
     * thing, only more efficiently.
     */
     ...
     */
    // @@ Mitchell, why is this method called "reap"?
    public ServerResponse reap() {
        // @@ Mitchell, could we please use the Java ThreadLocal
        // storage class for this stuff?

        // Get the calls made by the current thread.
        long threadId = Thread.currentThread().getId(); 
        List<PlatformCall> callObjects =
            mCallObjectMap.get(threadId);		
    ...
    private void appendCallObject(String fname, String... params) {
        ...
        callObjects.add(new PlatformCall(fname,
                                         paramList));
    }

    /**
     * Appends a call to outputLine to the current list of calls to be
     * returned to the client.
     */
    @Override
	public String outputLine(String line) {
        appendCallObject("outputLine", line);
        // @@ Mitchell, should this method return the line parameter?
        return null;
    }

    /**
     * Since all input is handled by the servlet, this method will
     * never be called.
     */
    @Override
    ...
    /**
     * Appends a call to outputString to the current list of calls to
    ...
    /**
     * This method is only called by the Android application and will
     * never be called in the context of this server.
     */
    @Override
    ...
    /**
     * Appends a call to outputMenu to the current list of calls to be
     * returned to the client.
     */
    @Override
    ...
    /**
     * Appends a call to enableOption to the current list of calls to
     * be returned to the client.
     */
    @Override
    ...
    /**
     * Appends a call to disableAll to the current list of calls to be
     * returned to the client.
     */
    @Override
    ...
    /**
     * Appends a call to addString to the current list of calls to be
     * returned to the client.
     */
    @Override
    ...
    /**
     * Appends a call to errorLog to the current list of calls to be
     * returned to the client.
     */
    @Override
